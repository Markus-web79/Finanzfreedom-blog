  ...
  categories.forEach((cat) => {
    ...
    if (fs.existsSync(contentDir)) {
      ...
      files.forEach((file) => {
        ...
        paths.push({
          params: {
            category: CATEGORY_CONFIG[cat].slug,
            slug,
          },
        });
      });
...
export async function getStaticProps({ params }) {
  ...
  const catKey = Object.keys(CATEGORY_CONFIG).find(
    (key) => CATEGORY_CONFIG[key].slug === category
  );

  const categoryData = CATEGORY_CONFIG[catKey];

  const filePath = path.join(process.cwd(), "content", catKey, `${slug}.md`);
