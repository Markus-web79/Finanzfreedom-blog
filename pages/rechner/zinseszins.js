...
export default function ZinseszinsRechner() {
  ...
  const startkapitalNum = Number(startkapital || 0);
  const monatlichNum = Number(monatlich || 0);
  const jahreNum = Number(jahre || 0);
  const renditeNum = Number(rendite || 0);

  const monate = jahreNum * 12;
  const monatszins = renditeNum / 100 / 12;

  let endkapital = startkapitalNum;

  for (let i = 0; i < monate; i++) {
    endkapital = endkapital * (1 + monatszins) + monatlichNum;
  }

  const eingezahlt = startkapitalNum + monatlichNum * monate;
      ...
      <main style={styles.page}>
        ...
        <section style={styles.note}>
          ...
          <p>
            ...
            auch die bereits erzielte Rendite arbeitet weiter für dich.
            Je länger dein Anlagezeitraum ist, desto stärker kann dieser
            Effekt werden.
          </p>
